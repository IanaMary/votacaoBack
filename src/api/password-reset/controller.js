...
export const create = ({ bodymen: { body: { email, link } } }, res, next) =>
  User.findOne({ email })
    ...
    .then((reset) => {
      ...
      const content = `
        Hey, ${user.name}.<br><br>
        You requested a new password for your meu-servidor-web account.<br>
        ...
        &mdash; meu-servidor-web Team
      `
      return sendMail({ toEmail: email, subject: 'meu-servidor-web - Password Reset', content })
    })
